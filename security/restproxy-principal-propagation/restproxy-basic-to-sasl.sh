...
docker-compose -f docker-compose.yml -f security/restproxy-principal-propagation/docker-compose-basic-sasl.yml up -d --build --no-deps restproxy
...
KAFKA_CLUSTER_ID=`curl -s -u client:client http://localhost:8082/v3/clusters |jq '.data'|jq -r '.[].cluster_id'`
echo "Kafka cluster id is $KAFKA_CLUSTER_ID"

echo
echo "Creat topic test"
curl -s -u client:client -X POST \
