...
resource "confluent_service_account" "metric-importer" {
  display_name = "metric-importer"
  description  = "Service account to export metrics of Kafka cluster"
}

resource "confluent_role_binding" "metric-importer" {
  principal   = "User:${confluent_service_account.metric-importer.id}"
