...
locals {
  hosted_zone = replace(regex("^[^.]+-([0-9a-zA-Z]+[.].*):[0-9]+$", confluent_kafka_cluster.pl-dedicated.bootstrap_endpoint)[0], "glb.", "")
}
...
locals {
  bootstrap = split(":", confluent_kafka_cluster.pl-dedicated.bootstrap_endpoint)[0]
}

locals {
  bootstrap_prefix = split(".", confluent_kafka_cluster.pl-dedicated.bootstrap_endpoint)[0]
}
...
resource "aws_security_group" "privatelink" {
  ...
  description = "Confluent Cloud Private Link minimal security group for ${confluent_kafka_cluster.pl-dedicated.bootstrap_endpoint} in ${var.vpc_id}"
