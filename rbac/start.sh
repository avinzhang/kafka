...
KAFKA_CLUSTER_ID=`confluent cluster describe --ca-cert-path ./secrets/ca.crt --url https://localhost:1090 -ojson|jq -r ".scope[].id"`
...
echo "   * Permission for Connect cluster"
confluent iam rbac role-binding create --kafka-cluster-id $KAFKA_CLUSTER_ID --principal Group:c3users --role ClusterAdmin --connect-cluster-id connect-cluster
echo "   * Permission for schema registry"
confluent iam rbac role-binding create --kafka-cluster-id $KAFKA_CLUSTER_ID --principal Group:c3users --role ClusterAdmin --schema-registry-cluster-id schema-registry
echo "   * Permission for ksqldb"
confluent iam rbac role-binding create --kafka-cluster-id $KAFKA_CLUSTER_ID --principal Group:c3users --role ClusterAdmin --ksql-cluster-id $KSQLDB_CLUSTER_ID
