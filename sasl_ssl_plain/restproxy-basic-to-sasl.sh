...
docker-compose -f docker-compose.yml -f ./restproxy-propagation/docker-compose-basic-sasl.yml up -d --build --no-deps restproxy
...
KAFKA_CLUSTER_ID=`curl -s --cacert ./secrets/snakeoil-ca-1.crt -u fred:letmein1 https://localhost:8082/v3/clusters |jq '.data'|jq -r '.[].cluster_id'`
echo "Kafka cluster id is $KAFKA_CLUSTER_ID"

echo
echo "Creat topic test"
curl -s --cacert ./secrets/snakeoil-ca-1.crt -u fred:letmein1 -X POST \
