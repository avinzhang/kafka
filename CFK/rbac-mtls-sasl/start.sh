...
for i in zookeeper kafka connect schemaregistry ksqldb controlcenter
  do
    ...
    --from-file=fullchain.pem=./secrets/$i.pem \
    --from-file=cacerts.pem=./secrets/ca.pem \
       --from-file=privkey.pem=./secrets/$i-key.pem
done
echo "Create secret for kafka sasl users"
...
kubectl create secret generic mds-token \
  --from-file=mdsPublicKey.pem=./secrets/mds/mds-publickey.txt \
  --from-file=mdsTokenKeyPair.pem=./secrets/mds/mds-tokenkeypair.txt
echo "Done"
echo 
echo ">>setup oauth credential for each component"
kubectl create secret generic mds-client \
  --from-file=bearer.txt=./secrets/kafka-client.txt
...
kubectl create secret generic rest-credential \
  --from-file=bearer.txt=./secrets/kafka-client.txt \
  --from-file=basic.txt=./secrets/kafka-client.txt
echo "Done"
...
kubectl exec -it connect-0 -c connect -- curl --cacert /mnt/sslcerts/cacerts.pem -u connectuser:connectuser -X POST -H "Content-Type: application/json" https://connect:8083/connectors/ \
...
kubectl exec -it connect-0 -c connect -- curl --cacert /mnt/sslcerts/cacerts.pem -u connectuser:connectuser https://connect:8083/connectors/datagen-users/status
kubectl exec -it connect-0 -c connect -- curl --cacert /mnt/sslcerts/cacerts.pem -u connectuser:connectuser https://connect:8083/connectors/datagen-pageviews/status
echo 
echo "------------------------------------------"
echo "Install ksqldb"
...
kubectl exec -it ksqldb-0 -c ksqldb -- bash -c 'cat << EOF > /tmp/client-ssl.properties
ssl.truststore.location=/mnt/sslcerts/truststore.p12
ssl.truststore.password=mystorepassword
EOF'
echo "Done"
...
kubectl exec -it  ksqldb-0 --  bash -c "ksql --config-file /tmp/client-ssl.properties -u ksqluser -p ksqluser https://ksqldb:8088 <<EOF
...
echo
echo 
echo ">> Apply role bindings for user1 to log into C3"
kubectl apply -f ./user1-c3.yaml
